...
ucl_lbl <- paste0 ("UCL: ",round(mean_close,2))
lcl_lbl <- paste0 ("LCL: ",round(mean_close,2))
...
p <- ggplot (data = df, aes(x = date, y = close)) +
  ...
  annotate("text", x = min(df$date), y = mean_close + 0.3, label = mean_lbl, hjust = 0, color = "red", size = 3.5) +
